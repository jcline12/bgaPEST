...
!*****************************************************************************************************
!****** Subroutine to calculate posterior covariance matrix (V)    ***********************************
!*****************************************************************************************************
subroutine bmo_calculate_V(cv_A,d_A,cv_OBS,d_XQR,cv_S,d_S,cv_PAR,d_PM,d_PAR,Q0_All,cv_PM,nQ0)
        implicit none
        ...
        type(cv_prior_mean),  intent(in)    :: cv_PM
        double precision                    :: invGyy(cv_OBS%nobs,cv_OBS%nobs)
        double precision                    :: Gyy(cv_OBS%nobs,cv_OBS%nobs)
        double precision                    :: Gss(cv_OBS%nobs,cv_OBS%nobs)
        double precision, pointer           :: TMPV(:)  
        double precision, pointer           :: XQbb(:,:)
        double precision, pointer           :: HXQbb(:,:)
        double precision, pointer           :: OMEGA(:,:)
        
    ! --------------- initializations
    invGyy = UNINIT_REAL ! matrix
   ...
   if (cv_PM%betas_flag.ne.0) then   !------> we have prior information about beta   
      ...
      ! Form Gss which is Qss + XQbbXt
     !***********************
     ! Marco - can you address the Q calculations here?
     !***********************
      
      
!      allocate(XQbb(cv_PAR%npar,cv_PAR%p))
!      call dgemm('n', 'n', cv_PAR%npar, cv_PAR%p, cv_PAR%p, 1.D0, d_XQR%X, &
!            cv_PAR%npar, d_PM%Qbb, cv_PAR%p, 0.D0,  XQbb, cv_PAR%npar)
!      ! now multiply XQbb by Xt and add the result (XQbbXt) to Qss, forming Gss
!      Gss = d_A%Qss
!      call dgemm('n', 't', cv_PAR%npar, cv_PAR%npar, cv_PAR%p, 1.D0, XQbb, &
!            cv_PAR%p, d_XQR%X, cv_PAR%npar, 1.D0,  Gss, cv_PAR%npar)
   ...
   !------ V = Gyy - Gsy(invGyy)GsyT

   !******************
   ! Marco - I think we need to make these calculations "on the fly" in cases where compression of Q is used
   !******************
   !Calculate the inverse of Gyy
   invGyy = Gyy   ! nobs x nobs
   call INVGM(cv_OBS%nobs,invGyy)

   ! Calculate Gsy
    

end subroutine bmo_calculate_V
!*****************************************************************************************************
!****** End Subroutine to calculate posterior covariance matrix (V)    *******************************
!*****************************************************************************************************
