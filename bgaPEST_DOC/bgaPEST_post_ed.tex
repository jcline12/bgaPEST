...
\renewcommand{\cooperationwith}{Groundwater Resources Program\newline{}
Global Change Research and Development}

\renewcommand{\reportbodypages}{86}

\renewcommand{\reportseries}{Techniques and Methods, Book 7, Section C9}
\renewcommand{\reportwebsiteremainder}{tm/tm7c9}
...
\renewcommand{\thetitlepageauthors}
...
\renewcommand{\theauthors}{Michael N. Fienen, Marco D'Oria, John E. Doherty, and Randall J. Hunt}
...
Approach (BGA%
\footnote[4]{Whereas the Bayesian Geostatistical Approach is referenced with the
all capital abbreviation ``BGA,''  lowercase is
...
approach of template and instruction files is omitted here;
...
\url{http://pubs.usgs.gov/tm/tm7c9}. This location includes a copy of this document and
both executables and source code. As development of the code continues,
a repository at \url{http://github.com/mnfienen-usgs/bgaPEST} provides a link to revisions in progress
...
at \url{http://pubs.usgs.gov/tm/tm7c9 }. 
...
of constructing a model and incorporating data;soft knowledge however, not all of
...
colocated in space and time. This correspondence is expressed as
...
and to allow the information contained in the calibration dataset
...
data and model outputs colocated in space and time with the measured
...
The forward model is constructed to generate outputs of values colocated
...
and correlation length) that control the roughness of distributed parameter
...
between model output and colocated observation data, given a candidate
...
implemented either using a script written by the user or employing a Python\footnote[5]
{``Python\textsuperscript{\textregistered}" is a registered trademark of the Python Software Foundation.}
...
\url{http://www.pesthomepage.org}. The necessary executable files are also
provided with bgaPEST. For users on the Windows\footnote[6]{``Windows\textsuperscript{\textregistered}" is a registered trademark of the Microsoft group of companies.} operating system,
...
For users on Macintosh\footnote[7]{``Macintosh\textsuperscript{\textregistered}" is a registered trademark of Apple, Inc. in the U.S. and other countries.} or Linux\footnote[8]{``Linux\textsuperscript{\textregistered}" is the registered trademark of Linus Torvalds in the U.S. and other countries.} systems, all the code must be compiled
...
with PEST and Python can be replaced by using the parallel external derivatives capabilities described in appendix \ref{sec:parallel}. Alternatively, it would be possible to implement the general
parallel run management suite (GENIE, \citet{GENIE}). 
...
on these data elements. As such, control variables operate on a different level from data variables and control either the reading/writing
...
to determine how to calculate $Q_{ss}$. {[}0{]} = no compression---calculate
full $Q_{ss}$ matrix, {[}1{]} = calculate separate $Q_{ss}$ matrix
...
\SECTION{Suggestions and Guidelines for \newline{}Initial Use}
...
of the method. We also offer a few guidelines to help users avoid potential
...
field, or surface flux of atmospheric gases. These parameters should
...
such codes can make use of them. Similarly, parallelization is supported to a limited degree using Python scripts and Condor \citep{Condor766} for run management. This parallel implementation is documented in appendix \ref{sec:parallel}.
...
implementation. For example, version 1.0 has a limited explicit parallelization
...
data to support a distributed parameter set are limited to part of
...
The authors thank Dr. Michael Cardiff, University of Wisconsin---Madison, Stephen Westenbroek, USGS Wisconsin Water Science
Center, and Mike Eberle, USGS Science Publishing Network, for valuable technical and editorial
 reviews of this document. The authors finally thank Dr. Peter Kitanidis, Stanford University, for his pioneering work on this technique and mentoring the first author as a graduate advisor.
...
\bibitem[{Condor Team(2012)}]{Condor766}
...
can become a problem for large and complicated datasets. The protocol
...
\item [{\texttt{Q\_compression\_flag}}] \emph{integer, default=0 }Flag
to determine how to calculate $\mathbf{Q}_{ss}$. {[}0{]} = no compression---calculate
full $\mathbf{Q}_{ss}$ matrix, {[}1{]} = Calculate separate $\mathbf{Q}_{ss}$ matrix
...
$\mathbf{Q}_{ss}$ matrix. {[}0{]} = do not consider anisotropy, {[}1{]}=consider
...
\texttt{model\_command\_lines} block, {[}4{]} = use external derivatives in parallel (see appendix \ref{sec:parallel} for details).
...
\item [{\texttt{beta\_0}}] \emph{float} Value of prior mean $\left(\beta_{0}\right)$
...
\begin{description}
\item [{\texttt{sig\_0}}] \emph{float }Initial value of the epistemic uncertainty variance
$\left(\sigma_{R_{0}}^{2}\right)$.
\item [{\texttt{sig\_opt}}] \emph{integer }Flag indicating whether epistemic
uncertainty variance should be optimized for or not. {[}0{]} = do not optimize, {[}1{]}
...
variance on $\sigma_{R_{0}}^{2}$. \texttt{sig\_p\_var = 0} means no prior variance
...
\item [{\texttt{grouptype}}] \emph{integer} Integer identifying which type of parameter the group corresponds to. This is used to ensure that beta associations do not span parameter types (for example, hydraulic conductivity parameters should not be in the same group type as recharge parameters). The specific values are arbitrary, but a distinct value should be assigned to each parameter group type.
\item [{\texttt{derinc}}] \emph{float} The derivative increment for calculating external derivates if using external derivatives calculation (\texttt{deriv\_mode = 0 or deriv\_mode = 4}).
\end{description}
...
\item [{\texttt{StartValue}}] \emph{float }Starting parameter value.
...
Currently, a single forward model command and an optional derivative
...
is the $n\times1$ vector of modeled forecasts colocated with observations
$\left(\mathbf{y}\right),$ and \textbf{$\mathbf{R}$ }is the $n\times n$
epistemic uncertainty covariance, modeled as $\sigma_{R}^{2}\mathbf{W}$
where $\sigma_{R}^{2}$ represents epistemic uncertainty, and $\mathbf{W}$
is an $n\times n$ diagonal matrix containing the observation weights. In general terms, the likelihood
...
\begin{equation}
\Phi_T=\mathbf{-}\frac{1}{2}\ln p\left(\mathbf{s|y}\right)=(\mathbf{s-X\beta}^{*})^{T}\mathbf{G_{\mathbf{ss}}^{-1}}(\mathbf{s-X\beta^{*}})-\frac{1}{2}\left(\mathbf{y-h}(\mathbf{s})\right)^{T}\mathbf{R^{-1}\left(\mathbf{y-h}(\mathbf{s})\right)}
\end{equation}
...
\parindent 0 in
where $\mathbf{s}_{opt}$ minimizes the objective function, $\Phi_T,$
using a Nelder-Mead simplex \citep[see, for example, ][]{Press1992}, which
guarantees monotonic decrease in $\Phi_T$ over successive
...
find a minimum value of $\Phi_T$ because the nonlinearity
...
system, and $x_{rot}$ and $y_{rot}$ constitute the location projected into
...
in parameter values indicative of geologic contacts (see, for example, \citealp{fienenEtal2004}; \citealp{FienenWRR2008}). Prior drift is accounted for in $\mathbf{X}$
...
\item the nugget
\begin{equation}
\mathbf{Q}_{ss}\left(\mathbf{d}\right)=\sigma^{2}
\end{equation}
and
\item the exponential covariance function 
\begin{equation}
\mathbf{R}_{ss}(\mathbf{d})=\sigma^{2}\mathrm{exp}(\mathbf{-}\frac{\left|\mathbf{d}\right|}{\ell})\label{eq:exp_GCF}
\end{equation}
...
as
\begin{equation}
\mathbf{Q_{ss}}(\mathbf{h},\sigma^2)=\sigma^2\ell\mathrm{exp}(-\frac{\mathbf{\left|\mathbf{d}\right|}}{\ell})\label{eq:Qss}.
\end{equation}
...
by the single free structural parameter $\sigma^2.$ The motivation
for this covariance function choice is to impart minimal assumptions
about parameter structure onto the solution. 

The appropriate values of $\mathbf{\theta}$ (the vector of structural parameters including $\sigma_{R}^2$ and $\sigma^2$) are calculated through restricted maximum likelihood.
...
of \citet{KitanidisVomvoris1983}, \citet{HoeksemaKitanidis1984a}, and \citet{Kitanidis1995},
...
are colocated spatially and temporally with the observation values,
...
\parindent 0 in
where $\mathbf{G_{yy}}$ is the measurement auto-covariance defined
as 
\begin{equation}
...
including $\det\left(\mathbf{Q}_{\mathbf{\theta\theta}}\right)^{-\frac{1}{2}}$.
 The optimal values for $\mathbf{\theta}$ are found by using the Nelder-Mead
...
the estimation of model parameters. This is accomplished through
a sequence of coupled inversion as follows.
\parindent 0.25 in 
\begin{enumerate}
\item Initialize model parameters as $\left(\mathbf{s_{0}}\right)$
and structural parameters $\left(\mathbf{\theta_{0}}\right)$. 
\item Solve for a new estimate of model parameters $\left(\hat{\mathbf{s}}\right)$
...
\begin{equation}
\mathbf{p}=\left(\frac{\mathbf{s}+\alpha}{\alpha}\right)^{\alpha}
\end{equation} \\
\parindent 0.25 in
...
and
\begin{equation}
\mathbf{p}=\lim_{\alpha \rightarrow \infty}=\left(\frac{\mathbf{s}+\alpha}{\alpha}\right)^{\alpha}=\exp\left(\mathbf{s}\right).
\end{equation}
...
In the case where compression of $\mathbf{Q}_{ss}$ is not used, the full
matrix $\mathbf{V}$ is calculated and reported. Where compression
of $\mathbf{Q}_{ss}$ is used, however, the diagonal of $\mathbf{V}$ is
...
%\end{bibunit}
%\begin{bibunit}
\APPENDIX{\label{sec:parallel}Parallel Implementation of Jacobian Calculations}

\subsection{Introduction}
A substantial computational burden in quasi-linear parameter estimation is the repeated calculation of the Jacobian matrix 
($\mathbf{H}$). Calculation of this matrix is ``embarassingly" or ``pleasingly" parallel---the number of runs required is the same as the number of parameters, but the individual model runs do not need to interact in any way. Given this property of Jacobian matrix calculation, a cluster of individual computers can be used to make the computations in parallel. Parallel implementation can be performed using a problem-specific implementation coded into the software \citep[for example,][]{beoPEST}, compiling the software against parallelization libraries \citep[for example,][]{GENIE}, or using a parallel management system outside the software \citep[for example,][]{Condor766}.

As an initial step toward full support of parallel Jacobian calculations, this appendix documents an implementation using the Condor package \citep{Condor766} and Python scripts. This implementation currently calculates the Jacobian matrix using forward finite-differences with the increment set by the user in the \texttt{derinc} variable in the \texttt{parameter\_groups} table (appendix \ref{sec:Input-Instructions}).

\subsection{Requirements}
Several requirements must be met to use this parallel implementation for Jacobian calculations.
\begin{enumerate}
\item Condor and the related program DAGman must be installed and active on both the master node where bgaPEST is to run and at least one worker node. Details for installation are available from \cite{Condor766}.
\item Python, including the module numpy, must be installed on all compute nodes that will be used by Condor, including the master node where bgaPEST is running.
\item In the folder where bgaPEST is running, the following files must be present
\begin{itemize}
\item Condor\_ATC.py
\item parallel\_condor\_Jacobian.py
\item jacobian\_pre.py
\item jacobian\_pre.bat (or jacobian\_pre.sh on non-Windows operating systems)
\item jacobian\_post.py
\item jacobian\_post.bat (or jacobian\_post.sh on non-Windows operating systems)
\item condor\_jacobian.sub.orig
\item unzip.exe
\item zip\_results.py
\end{itemize}
\item A folder called ``data" must be present in the same folder where bgaPEST is running on the master node. The ``data" folder must contain all forward-model related files, the \texttt{.bgp} file, and all \texttt{.tpl} and \texttt{.ins} files used by bgaPEST. Additionally, the following files must be present
\begin{itemize}
\item tempchek.exe (included in example files or available from \url{http://www.pesthomepage.org})
\item inschek.exe (included in example files or available from \url{http://www.pesthomepage.org})
\item condor\_single\_run.py
\item parallel\_condor\_Jacobian.py

\end{itemize}

\item Set the variable \texttt{deriv\_mode=4} in the \texttt{algorithmic\_cv} Keywords input block (appendix \ref{sec:Input-Instructions}). This instructs bgaPEST to write necessary data transfer files and to execute the code necessary to run the Condor-based parallel Jacobian calculation.
\end{enumerate}


\subsection{File modifications}
The file \texttt{condor\_jacobian.sub.orig} must be customized to contain instructions specific to the Condor network on which bgaPEST is running for the \texttt{requirements}  and the \texttt{request\_memory} variables. Details of these variables are described by \cite{Condor766}. All other variables should remain unchanged from the files provided with this documentation.

\subsection{Description of the Method}
This parallel implementation of bgaPEST using Condor is a scripted approach using a combination of existing PEST utilities and custom Python code. The general approach consists of the following steps:
\begin{enumerate}
\item Files are written by bgaPEST to communicate to the parallel codes information specific to the current run including derivative increments (specified by the user in the \texttt{derinc} variable).
\item At the time of each Jacobian calculation, the current parameter values are written to a temporary file
\item The current parameter values are combined with all other necessary model files in the ``data" subfolder, which is compressed into a zipfile.
\item The Condor submit file is updated to specify the correct number of model runs required to calculate the Jacobian matrix.
\item A single-node directed acyclic graph (DAG) is initiated using the Condor utility \texttt{DAGman}. The need for the DAG is to monitor the set of Condor jobs corresponding to the specific model runs. \texttt{DAGman} monitors the progress of the DAG and returns control to bgaPEST ocne the entire Jacobian is calculated.
\item On each worker node, the following tasks are performed:
	\begin{itemize}
	\item Model input files are written by using the \texttt{tempchek.exe} utility
	\item The model is run once using the model command line information provided by bgaPEST
	\item The model output files are read by using the \texttt{inschek.exe} utility. The version of \texttt{inschek.exe} provided with bgaPEST has been modified to carry maximum numerical precision.
\end{itemize}
\item After the model is run for each perturbation and a base case, the derivatives are calculated using forward differences and conveyed back to bgaPEST by using a text file.
\end{enumerate}
\subsection{References Cited}
\begin{thebibliography}{17}
\providecommand{\natexlab}[1]{#1}
...
21 rows and 21 columns and with constant row and column spacing of 1 meter (m).
...
estimated hydraulic conductivity field and squared differences between
measured and observed head values, respectively, for case 4. In this
...
to conform to such a shape, so its use should be approached with
caution.
...
% ACRONYMS
\glosssubsection{Mathematical Symbols and Variables}

\item[$\emph{m}$] Number of parameters
\item[$\emph{n}$] Number of observations
\item[$\emph{p}$] Number of beta associations
\item[$\mathbf{s}$] Vector of parameters
\item[$\mathbf{s}_0$] Starting values of parameters
\item[$\mathbf{y}$] Vector of observations
\item[$\mathbf{y}_{k}^{'}$] Observations corrected for the $k^{th}$ linearization:  $\mathbf{y}_{k}^{'}=\mathbf{y}-\mathbf{h}\tilde{\mathbf{s}}_{k}+\tilde{\mathbf{H}}_{k}\tilde{\mathbf{s}}_{k}$
\item[$\mathbf{h}\left(\mathbf{s}\right)$] Modeled results of $\mathbf{s}$
\item[$\mathbf{H}$] Jacobian matrix of parameter sensitivities
\item[$\mathbf{W}$] Diagonal weight matrix
\item[$\sigma^2_R$] Epistemic uncertainty
\item[$\sigma^2$] Variogram variance or slope parameter
\item[$\mathbf{R}$] Epistemic uncertainty covariance
\item[$p\left(\mathbf{s}\right)$] Prior probability of parameter values
\item[$L\left(\mathbf{y}|\mathbf{s}\right)$] Likelihood function
\item[$p\left(\mathbf{s}|\mathbf{y}\right)$] Posterior probability of parameter values
\item[$\mathbf{X}$] A selection matrix that can include drift of the prior mean
\item[$\beta$] Prior mean values for each beta association
\item[$\beta^*$] Initial prior mean values for each beta association used in the prior pdf
\item[$\beta_0$] Starting prior mean values for each beta association
\item[$\theta$] Vector of the structural parameters
\item[$\hat{\theta}$] Best estimate of the vector of the structural parameters
\item[$\theta^*$] Initial value of structural parameters (used in the prior pdf of $\theta$)
\item[$\theta_0$] Starting value of the structural parameters
\item[$p\left(\mathbf{\theta}\right)$] Prior probability of structural parameter values
\item[$L\left(\mathbf{y}_{k}^{'}|\theta\right)$] Likelihood function in structural parameter optimization
\item[$p\left(\mathbf{\theta|\mathbf{y}_{k}^{'}}\right)$] Posterior probability of sturctural parameters given the current data
\item[$\mathbf{G}_{ss}$] Prior covariance of $\left(\mathbf{s}-\mathbf{X}\beta^*\right)$
\item[$\mathbf{G}_{sy}$] Cross covariance between observations and parameters
\item[$\mathbf{G}_{yy}$] Auto-covariance of the observations when covariance of the prior mean is included
\item[$\mathbf{Q}_{ss}$] Prior covariance of the parameters
\item[$\mathbf{Q}_{yy}$] Auto-covariance of the observations
\item[$\mathbf{Q}_{\beta\beta}$] Prior covariance of the prior mean values
\item[$\mathbf{Q}_{\theta\theta}$] Prior covariance of the structural parameter values
\item[$\mathbf{\tilde{s}}$] Current best estimate of $\mathbf{s}$ during quasi-linear runs
\item[$\mathbf{\tilde{H}}$] Current sensitivity matrix, a function of $\mathbf{\tilde{s}}$ during quasi-linear runs
\item[$\mathbf{\xi}$] Interpolation weights for the innovation in solving for $\mathbf{\hat{s}}$
\item[$\mathbf{\hat{s}}$] Best estimate of $\mathbf{s}$
\item[$\mathbf{\hat{\beta}}$] Best estimate of $\mathbf{\beta}$
\item[$\Phi_T$] Total objective function 
\item[$\Phi_M$] Measurement component of objective function 
\item[$\Phi_R$] Regularization component of the objective function 
\item[$\Phi_S$] Structural parameters objective function 
\item[$\mathbf{s}_{\text{opt}}$] Optimal value of parameters in the linesearch
\item[$\rho$] Linesearch parameter
\item[$E{\left[\cdot\right]}$] Expected value
\item[$\mathbf{d}$] Separation distance in variogram calculation
\item[$\ell$] Integral scale in variogram calculation
\item[$\mathbf{v}$] Vector of epistemic uncertainty terms
\item[$\mathbf{W}$] Observation weight matrix
\item[$\mathbf{T}$] An arbitrary block Toeplitz matrix
\item[$\mathbf{\omega}$] Observation weight
\item[$\mathbf{p}$] Untransformed parameter values in the context of power transformation
\item[$\alpha$] Exponent for power transformation
\item[$\mathbf{V}$] Posterior covariance matrix of the parameters
\end{usgsglossary}
\end{document}
