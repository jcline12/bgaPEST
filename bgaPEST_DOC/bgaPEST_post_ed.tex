...
such codes can make use of them. Similarly, parallelization supported to a limited degree using Python scripts and Condor \citep{Condor766} for run management. This parallel implementation is documented in Appendix \ref{sec:parallel}.
...
\item [{\texttt{derinc}}] \emph{float} The derivative increment for calculating external derivates if using 
parallel, external derivatives calculation (\texttt{deriv\_mode = 4}).
\end{description}
...
\item[$p\left(\mathbf{R}\right)$] Epistemic uncertainty covariance
...
\item[$\beta^*$] Vector of prior mean values for each beta association
