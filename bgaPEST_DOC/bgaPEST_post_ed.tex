...
Approach (BGA%
\footnote[4]{Whreas the Bayesian Geostatistical Approach is references with the
...
approach of template and instruction files are ommitted here;
...
The variogram used in riging is an empirical function that characterizes
...
the site-specific observations. This idea is also inspired by Chamerlin's
...
log-transformed parameters, the upper and lower 95 percent percent confidence
...
will be the number of parmeters $\left(m\right)$ $\times$ Run Time. 
...
grid spacing can also be relaxed by using a Karhunen-Loeve transform
...
approximately 2 orders of magnitude higher thanthe artificial noise
...
the parameter field is better in case 6 with anisotropy than in case
...
adopted. The upstream and downstream boundary conditions were a streamflow
