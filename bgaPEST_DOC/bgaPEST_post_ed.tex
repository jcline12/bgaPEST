...
suite of software \citet{PEST4}. This report provides details on the
...
The Bayesian geostatistical approach is described in detail in \citet{KitanidisVomvoris1983}, \citet{HoeksemaKitanidis1984a}, \citet{Kitanidis1995}, and \citet{NowakCirpka2004}
...
Partitioning the field based on either the data \citep[for example, ][]{fienenEtal2004}
...
\texttt{PEST2} is used to store matrices: when posterior covariance
...
to be read by \texttt{bgaPEST}. If \texttt{deriv\_mode=0}, \texttt{PEST2}
...
used by \texttt{PEST2}, as described in \citet{PESTAdd2}, Section 4.4.3.
An example and description of the format of a standard \texttt{PEST2}
...
\caption{Example of a standard \texttt{PEST2} matrix file, adapted from \citet{PESTAdd2}}
\label{fig:matrix}
\end{centering}
\end{figure*}
...
Details of the contruction of these files are in \citet[][chap. 3]{PEST3}.
...
about whether to transform the value with a logarithmic or power transform
...
in the $\mathbf{\theta}$ parameters can be enforced using a power transformation
