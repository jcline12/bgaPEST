...
! -- Program JACWRIT translates a binary Jacobian "jco" file to an ascii file.
        ...
        CHARACTER*120 JACFLE
        CHARACTER*200 COMLIN


        DOUBLE PRECISION, pointer, intent(out) :: X(:,:)
        CHARACTER*12 APAR(:)
        CHARACTER*12 AOBS1(:)
        CHARACTER*20 AOBS(:)
        ...
        OPEN(UNIT=9988,FILE=JACFLE,FORM='binary', STATUS='OLD',IOSTAT=IERR)
        IF(IERR.NE.0)THEN
          WRITE(*,125) JACFLE(1:NBLNK(JACFLE))
125       FORMAT(/,' *** Cannot open file ',A,' ***',/)
          GO TO 9990
        END IF
         READ(9988,ERR=9000,END=9100)NESPAR,NXROW
        IF(NESPAR.LT.0)THEN
          NESPAR=-NESPAR
          NXROW=-NXROW
          NEWFLAG=1
...
200       FORMAT(/,' *** Cannot allocate sufficient memory to ','continue execution ***',/)
        ...
        IF(NEWFLAG.EQ.1)THEN
          X=0.0D0                ! AN ARRAY
          READ(9988,ERR=9000,END=9100)ICOUNT
          DO I=1,ICOUNT
            READ(9988,ERR=9000,END=9100) J,DTEMP
            IES=(J-1)/NXROW+1
            IROW=J-(IES-1)*NXROW
            X(IROW,IES)=DTEMP
          END DO
        ELSE
          READ(9988,ERR=9000,END=9100) ((X(J,I),J=1,NXROW),I=1,NESPAR)
        END IF
        DO IPP=1,NESPAR
          READ(9988,ERR=9000,END=9100) APAR(IPP)
        END DO
        IF(NEWFLAG.EQ.1)THEN
          DO IOBS=1,NXROW
            READ(9988,ERR=9000,END=9100) AOBS(IOBS)
          END DO
        ELSE
          DO IOBS=1,NXROW
            READ(9988,ERR=9000,END=9100) AOBS1(IOBS)
